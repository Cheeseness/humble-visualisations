<?php
	...
	*/
	...
	parseData("http://www.humblebundle.com/weekly");
	...
	*/
	function parseData($url)
	{
		...
		$nodes = $pathfinder->query("//div[ contains (@class, 'weekly-over') ]");
		print("Hello?");	
		print(count($nodes));
		if (count($nodes) > 0)
		{
		...
		if ($isOver == false)
		{
			$query = "insert into scrapedata_weekly (bundleTitle, lastUpdated, paymentTotal, purchaseTotal, pcLin, pcMac, pcWin, paymentAverage, avLin, avMac, avWin, fullPrice) values ('" . htmlspecialchars($bundleTitle, ENT_QUOTES) . "', utc_timestamp(), '" . $paymentTotal . "', '" . $purchaseTotal . "', '" . $pcLin . "', '" . $pcMac . "', '" . $pcWin . "', '" . $paymentAverage . "', '" . $avLin . "', '" . $avMac . "', '" . $avWin . "', '" . $fullPrice . "')";
