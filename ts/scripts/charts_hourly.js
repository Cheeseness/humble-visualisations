...
{	
	...
	d3.json("getdata.php?bundle=" + bundleTitle, function(result) {
		...
		{
			if ((result.bundleTitle == "The Humble Double Fine Bundle") && (firstRow))
			{
				firstRow = false;
				continue;
			}
			//TODO: If there's more than an hour's gap between this record and the last, we need to insert an empty one so that the line graphs know to break.
...
var makePurchaseDiffChart = function(data)
{
	...
	var d2 = d3.layout.stack()(["puLinDiff", "puMacDiff", "puWinDiff"].map(function(p) { return data.map(function(d) { return {x: d.lastUpdated, y: +d[p], t: getShortTitle(data.bundleTitle), c: p, a: +d.puLinDiff + d.puMacDiff + d.puWinDiff}; }); }));
	console.log("Making value chart");
...
var makeRevenueDiffChart = function(data)
{
	...
	var d2 = d3.layout.stack()(["pyLinDiff", "pyMacDiff", "pyWinDiff"].map(function(p) { return data.map(function(d) { return {x: d.lastUpdated, y: +d[p], t: getShortTitle(data.bundleTitle), c: p, a: +d.pyLinDiff + d.pyMacDiff + d.pyWinDiff}; }); }));
	console.log("Making value chart");
...
{
	var chartTarget = d3.select("#chartPlayground").append("div").attr("class", "widestats");
	chartTarget.append("h2").html(data.bundleTitle + "<br />Hourly Per Purchase Totals");
