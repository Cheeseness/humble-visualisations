...
*/
	...
	*/
	function getData()
	{
		...
		$query = "select bundleTitle, lastUpdated, unix_timestamp(lastUpdated) as ts, paymentTotal, purchaseTotal, pcLin, pcMac, pcWin, paymentAverage, avLin, avMac, avWin, fullPrice, firstPrice, round((unix_timestamp((lastUpdated)) - unix_timestamp(firstStamp))/60/60) as hour from (select * from scrapedata2 where bundleTitle = (select bundleTitle from scrapedata2 order by lastUpdated desc limit 1)) as currentBundle , (select min(lastUpdated) as firstStamp, min(fullPrice) as firstPrice from scrapedata2 where bundleTitle = (select bundleTitle from scrapedata2 order by lastUpdated desc limit 1)) as minDate order by lastUpdated asc";
